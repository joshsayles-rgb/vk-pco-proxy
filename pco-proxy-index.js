...
async function getCheckInCounts(eventId) {
  ...
  let allCheckIns = [];
  let path = '/check-ins/v2/check_ins?where[event_period_id]=' + period.id + '&include=locations&per_page=100';
  ...
  console.log('Period:', period.id, period.attributes.starts_at);
  console.log('Total check-ins:', allCheckIns.length);
  console.log('First CI locations:', JSON.stringify(firstCI?.relationships?.locations));
  ...
  return {
    ...
    debug: {
      periodId: period.id,
      firstCILocations: firstCI?.relationships?.locations,
    }
  };
}
