...
console.log('Auth header start:', auth.substring(0, 20));
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({
      ok: true,
      appIdLength: PCO_APP_ID.length,
      secretLength: PCO_SECRET.length,
      authHeader: auth, // show full auth header for debugging
    }));
    return;
  }
